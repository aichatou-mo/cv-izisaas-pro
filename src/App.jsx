...
export default function App() {
  ...
  // payTab: 'mobile' | 'paypal' | 'virement'
  const [payTab, setPayTab] = useState('mobile');
  ...
  // CinetPay Mobile Money Afrique — via Vercel Serverless /api/cinetpay-init
  const submitCinetPay = async () => {
    if (!buyerName || !buyerPhone || !buyerEmail) {
      setPayError('Veuillez renseigner votre nom, téléphone mobile et email pour recevoir votre confirmation.');
      return;
    }
    ...
    try {
      ...
      if (data.success && data.paymentUrl) {
        ...
        setIsProcessingPay(false);
        setPayError(data.message || 'Erreur lors de l\'initialisation CinetPay. Contactez le support.');
      }
    } catch (err) {
      setIsProcessingPay(false);
      if (err.name === 'AbortError') {
        setPayError('Délai de connexion dépassé. Essayez l\'option PayPal ou Virement ci-dessus, ou contactez le support.');
      } else {
        // Serveur API non disponible (dev local sans vercel dev)
        setConfigPending(true);
        setPayError('');
      }
    }
  ...
  // PayPal — ouvre le lien de paiement PayPal.me ou PayPal Invoicing
  const submitPayPal = () => {
    if (!buyerName || !buyerEmail) {
      setPayError('Veuillez renseigner votre nom et email avant de continuer.');
      return;
    }
    // Lien PayPal.me ou PayPal Invoice — l'utilisateur remplace par son lien réel
    const paypalLink = `https://www.paypal.com/paypalme/aichatouMO/${currentPlan.eur}`;
    window.open(paypalLink, '_blank');
    // Notifier via WhatsApp avec les infos de l'acheteur
    const msg = `Bonjour Aïchatou, je viens de régler via PayPal.\nNom: ${buyerName}\nEmail: ${buyerEmail}\nPlan: ${currentPlan.name} (${currentPlan.eur} EUR)`;
    setTimeout(() => {
      window.open(`https://wa.me/22777063837?text=${encodeURIComponent(msg)}`, '_blank');
    }, 1500);
  };
...
<body>
  ...
  return (
    <div className="relative min-h-screen text-slate-900 selection:bg-brand-gold selection:text-slate-950 bg-[#FAFAF8] overflow-x-hidden font-sans">
      ...
      {/* MODAL 1 : GUICHET DE PAIEMENT MULTI-METHODES */}
      {isPayModalOpen && (
            ...
            {paySuccessData ? (
              ...
              <>
                ...
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="font-mono text-xs font-bold text-brand-goldDark tracking-wider uppercase">Paiement SÃ©curisÃ© SSL 256 bits</span>
                </div>

                <h3 className="text-xl sm:text-2xl font-bold text-slate-900 mb-1">Choisissez votre mode de paiement</h3>
                <p className="text-xs text-slate-500 mb-4">Paiement direct &amp; sÃ©curisÃ©. Aucun numÃ©ro marchand n'est exposÃ© ici.</p>

                {/* Plan Summary */}
                <div className="p-3.5 rounded-2xl bg-amber-50/80 border border-brand-gold/30 mb-5 flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-[11px] text-emerald-700 font-semibold flex items-center gap-1 mt-0.5">
                      <Lock className="w-3 h-3" /> Chiffrement bancaire SSL
                    </p>
                  </div>
                  <div className="text-right shrink-0 ml-3">
                    <p className="font-bold text-base text-slate-950">{currentPlan.fcfa.toLocaleString('fr-FR')} FCFA</p>
                    <p className="text-[11px] text-slate-500 font-mono">~{currentPlan.eur} EUR</p>
                  </div>
                </div>

                {/* Payment Method Tabs */}
                <div className="flex rounded-2xl bg-slate-100 p-1 gap-1 mb-5">
                  {[
                    { id: 'mobile', label: 'Mobile Money', icon: <Smartphone className="w-3.5 h-3.5" /> },
                    { id: 'paypal', label: 'PayPal', icon: <Globe className="w-3.5 h-3.5" /> },
                    { id: 'virement', label: 'Virement', icon: <Banknote className="w-3.5 h-3.5" /> },
                  ].map((tab) => (
                    <button
                      key={tab.id}
                      onClick={() => { setPayTab(tab.id); setPayError(''); setConfigPending(false); }}
                      className={`flex-1 py-2 rounded-xl text-[11px] font-bold transition-all flex items-center justify-center gap-1.5 ${
                        payTab === tab.id ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'
                      }`}
                    >
                      {tab.icon}
                      <span>{tab.label}</span>
                ...
                {/* ===== TAB 1: MOBILE MONEY PAN-AFRICAIN (CinetPay) ===== */}
                {payTab === 'mobile' && (
                  <div>
                    <p className="text-[11px] font-mono font-bold text-slate-600 uppercase mb-2">Couverture pan-africaine via CinetPay :</p>
                    <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5 mb-4">
                      {[
                        { color: 'bg-red-600', name: 'Airtel Money', sub: 'Niger, Rwanda' },
                        { color: 'bg-yellow-400', name: 'MTN MoMo', sub: 'Nigeria, Ghana' },
                        { color: 'bg-cyan-500', name: 'Wave', sub: 'SÃ©nÃ©gal, CI' },
                        { color: 'bg-orange-500', name: 'Orange/Moov', sub: 'UEMOA' },
                        { color: 'bg-green-600', name: 'M-Pesa', sub: 'Kenya, TZ' },
                        { color: 'bg-slate-700', name: 'Carte CB', sub: 'Visa / MC' },
                      ].map((m) => (
                        <div key={m.name} className="p-2 rounded-xl border border-slate-200 bg-slate-50 flex flex-col items-center gap-0.5 text-center">
                          <div className={`w-2.5 h-2.5 rounded-full ${m.color} mb-0.5`}></div>
                          <span className="text-[10px] font-bold text-slate-800 leading-tight">{m.name}</span>
                          <span className="text-[9px] text-slate-400 font-mono leading-tight">{m.sub}</span>
                        </div>
                      ))}
                    </div>
                    ...
                    <div className="space-y-2.5 pt-3 border-t border-slate-100">
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                        <div>
                          <label className="block text-xs font-semibold text-slate-700 mb-1">Nom &amp; PrÃ©nom *</label>
                          <input type="text" value={buyerName} onChange={(e) => setBuyerName(e.target.value)} placeholder="Ex: Ibrahim Ousmane" className="w-full px-3 py-2 rounded-xl border border-slate-300 text-xs focus:border-brand-gold focus:outline-none" />
                        </div>
                        <div>
                          <label className="block text-xs font-semibold text-slate-700 mb-1">TÃ©lÃ©phone Mobile *</label>
                          <input type="tel" value={buyerPhone} onChange={(e) => setBuyerPhone(e.target.value)} placeholder="+227 90 00 00 00" className="w-full px-3 py-2 rounded-xl border border-slate-300 text-xs font-mono focus:border-brand-gold focus:outline-none" />
                        </div>
                      </div>
                      <div>
                        <label className="block text-xs font-semibold text-slate-700 mb-1">Email * (reÃ§u &amp; accÃ¨s)</label>
                        <input type="email" value={buyerEmail} onChange={(e) => setBuyerEmail(e.target.value)} placeholder="votre.email@gmail.com" className="w-full px-3 py-2 rounded-xl border border-slate-300 text-xs focus:border-brand-gold focus:outline-none" />
                      </div>
                    </div>

                    {configPending && (
                      <div className="mt-3 p-3 rounded-2xl bg-amber-50 border border-amber-200 text-xs text-amber-900 space-y-1.5">
                        <p className="font-bold flex items-center gap-1.5"><ShieldCheck className="w-3.5 h-3.5 text-amber-700 shrink-0" /> CinetPay â€” ClÃ©s Vercel Ã  configurer</p>
                        <p className="text-[11px] leading-relaxed text-amber-800">
                          Le backend est prÃªt. Ajoutez <code className="bg-amber-100 px-1 rounded">CINETPAY_API_KEY</code> et <code className="bg-amber-100 px-1 rounded">CINETPAY_SITE_ID</code> dans <strong>Vercel â†’ Settings â†’ Environment Variables</strong>. En attendant, utilisez <strong>PayPal</strong> ou <strong>Virement</strong>.
                        </p>
                        <button onClick={() => setPayTab('paypal')} className="flex items-center gap-1 text-amber-800 font-bold underline text-[11px]">
                          <ArrowRight className="w-3 h-3" /> Basculer sur PayPal
                        </button>
                      </div>
                    )}

                    {payError && <div className="mt-3 p-3 rounded-2xl bg-red-50 border border-red-200 text-xs text-red-800">{payError}</div>}

                    <div className="mt-4 space-y-2">
                      <button onClick={submitCinetPay} disabled={isProcessingPay} className="magnetic-btn w-full py-3.5 rounded-full bg-gradient-to-r from-brand-gold to-[#E2B755] text-slate-950 font-bold text-sm flex items-center justify-center gap-2 shadow-lg shadow-brand-gold/30 hover:brightness-105 disabled:opacity-60 transition-all">
                        <ShieldCheck className="w-4 h-4 text-slate-950" />
                        <span>{isProcessingPay ? 'Connexion sÃ©curisÃ©e...' : `Payer Mobile Money (${currentPlan.fcfa.toLocaleString('fr-FR')} FCFA)`}</span>
                      </button>
                      <button onClick={handleSupportContact} className="w-full py-2.5 rounded-full bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold text-xs flex items-center justify-center gap-2 transition-colors">
                        <svg className="w-3.5 h-3.5 fill-[#25D366]" viewBox="0 0 24 24"><path d="M.057 24l1.687-6.163c-1.041-1.804-1.588-3.849-1.587-5.946.003-6.556 5.338-11.891 11.893-11.891 3.181.001 6.167 1.24 8.413 3.488 2.245 2.248 3.481 5.236 3.48 8.414-.003 6.557-5.338 11.892-11.893 11.892-1.99-.001-3.951-.5-5.688-1.448l-6.305 1.654zm6.597-3.807c1.676.995 3.276 1.591 5.392 1.592 5.448 0 9.886-4.434 9.889-9.885.002-5.462-4.415-9.89-9.881-9.892-5.452 0-9.887 4.434-9.889 9.884-.001 2.225.651 3.891 1.746 5.634l-.999 3.648 3.742-.981zm11.387-5.464c-.074-.124-.272-.198-.57-.347-.297-.149-1.758-.868-2.031-.967-.272-.099-.47-.149-.669.149-.198.297-.768.967-.941 1.165-.173.198-.347.223-.644.074-.297-.149-1.255-.462-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.297-.347.446-.521.151-.172.2-.296.3-.495.099-.198.05-.372-.025-.521-.075-.148-.669-1.611-.916-2.206-.242-.579-.487-.501-.669-.51l-.57-.01c-.198 0-.52.074-.792.372s-1.04 1.016-1.04 2.479 1.065 2.876 1.213 3.074c.149.198 2.095 3.2 5.076 4.487.709.306 1.263.489 1.694.626.712.226 1.36.194 1.872.118.571-.085 1.758-.719 2.006-1.413.248-.695.248-1.29.173-1.414z"/></svg>
                ...
                {payTab === 'paypal' && (
                  <div>
                    <div className="p-4 rounded-2xl bg-[#003087]/5 border border-[#003087]/20 mb-4">
                      <div className="flex items-center gap-1 mb-2">
                        <span className="font-extrabold text-[#003087] text-xl leading-none">Pay</span>
                        <span className="font-extrabold text-[#009CDE] text-xl leading-none">Pal</span>
                        <span className="text-xs text-slate-500 ml-2">Â· International Â· 200+ pays</span>
                      </div>
                      <p className="text-xs text-slate-600 leading-relaxed">
                        RÃ©glez <strong>{currentPlan.eur} EUR</strong> (~{currentPlan.fcfa.toLocaleString('fr-FR')} FCFA) via votre compte PayPal ou carte bancaire internationale (Visa, Mastercard, Amex).
                      </p>
                    </div>
                    ...
                    <div className="p-3 rounded-xl bg-blue-50 border border-blue-100 text-xs text-blue-800 mb-4 space-y-1">
                      <p className="font-bold">Ã‰tapes :</p>
                      <p>â‘  Cliquez "Payer via PayPal" â†’ redirection vers PayPal.me sÃ©curisÃ©</p>
                      <p>â‘¡ RÃ©glez <strong>{currentPlan.eur} EUR</strong> sur la page PayPal officielle</p>
                      <p>â‘¢ Un message WhatsApp de confirmation est envoyÃ© automatiquement</p>
                      <p>â‘£ Vos accÃ¨s sont dÃ©bloquÃ©s dans les 24h aprÃ¨s vÃ©rification</p>
                    </div>

                    {payError && <div className="mb-3 p-3 rounded-2xl bg-red-50 border border-red-200 text-xs text-red-800">{payError}</div>}

                    <div className="space-y-2">
                      <button onClick={submitPayPal} className="magnetic-btn w-full py-3.5 rounded-full bg-[#003087] hover:bg-[#00205c] text-white font-bold text-sm flex items-center justify-center gap-2 shadow-lg shadow-[#003087]/30 transition-all">
                        <Globe className="w-4 h-4" />
                        <span>Payer via PayPal ({currentPlan.eur} EUR)</span>
                        <ArrowRight className="w-4 h-4" />
                      </button>
                      <p className="text-[10px] text-slate-400 text-center font-mono">âš ï¸ Mettez Ã  jour l'URL PayPal.me avec votre vrai compte PayPal</p>
                    </div>
                  </div>
                )}

                {/* ===== TAB 3: VIREMENT BANCAIRE ===== */}
                {payTab === 'virement' && (
                  <div>
                    <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200 mb-4 space-y-2.5">
                      <div className="flex items-center justify-between mb-1">
                        <div className="flex items-center gap-2">
                          <Banknote className="w-4 h-4 text-slate-700" />
                          <span className="font-bold text-slate-900 text-sm">CoordonnÃ©es Bancaires</span>
                        </div>
                        <span className="text-[10px] font-mono text-slate-500 bg-slate-200 px-2 py-0.5 rounded-full">SWIFT / IBAN</span>
                      </div>
                      {[
                        { label: 'BÃ©nÃ©ficiaire', value: 'MOUSSA OUSMANE AICHATOU', field: 'bname' },
                        { label: 'Banque', value: 'Banque Commerciale du Niger (BCN)', field: 'bank' },
                        { label: 'IBAN / NÂ° Compte', value: 'NE00 0000 0000 0000 0000 0000 00', field: 'iban' },
                        { label: 'Code SWIFT / BIC', value: 'BCNENINIM', field: 'swift' },
                        { label: 'RÃ©fÃ©rence paiement', value: currentPlan.name.substring(0, 30), field: 'ref' },
                        { label: 'Montant exact', value: `${currentPlan.eur} EUR`, field: 'amount' },
                      ].map((item) => (
                        <div key={item.field} className="flex items-center justify-between gap-2 py-1.5 border-b border-slate-100 last:border-0">
                          <div>
                            <p className="text-[10px] text-slate-400 font-mono uppercase tracking-wide">{item.label}</p>
                            <p className="text-xs font-semibold text-slate-900 font-mono">{item.value}</p>
                          </div>
                          <button onClick={() => copyToClipboard(item.value, item.field)} className="shrink-0 flex items-center gap-1 text-[10px] font-mono text-brand-goldDark hover:text-amber-700 transition-colors px-2 py-1 rounded-lg hover:bg-amber-50">
                            {copiedField === item.field ? <CheckCircle className="w-3 h-3 text-emerald-600" /> : <Copy className="w-3 h-3" />}
                            <span>{copiedField === item.field ? 'CopiÃ© !' : 'Copier'}</span>
                          </button>
                        </div>
                      ))}
                    </div>
                    ...
                    <div className="p-3 rounded-xl bg-amber-50 border border-amber-200 text-xs text-amber-900 mb-4">
                      <p className="font-bold mb-1">âš ï¸ AprÃ¨s le virement :</p>
                      <p>Envoyez votre preuve de paiement Ã  AÃ¯chatou via WhatsApp ou email. Activation sous 24â€“48h ouvrÃ©es.</p>
                    </div>

                    <div className="space-y-2">
                      <button
                        onClick={() => {
                          const msg = `Bonjour AÃ¯chatou, j'ai effectuÃ© un virement bancaire pour : ${currentPlan.name} (${currentPlan.eur} EUR). Je vous envoie la preuve de paiement.`;
                          window.open(`https://wa.me/22777063837?text=${encodeURIComponent(msg)}`, '_blank');
                        }}
                        className="magnetic-btn w-full py-3.5 rounded-full bg-gradient-to-r from-brand-gold to-[#E2B755] text-slate-950 font-bold text-sm flex items-center justify-center gap-2 shadow-lg shadow-brand-gold/30 hover:brightness-105 transition-all"
                      >
                        <svg className="w-4 h-4 fill-slate-950" viewBox="0 0 24 24"><path d="M.057 24l1.687-6.163c-1.041-1.804-1.588-3.849-1.587-5.946.003-6.556 5.338-11.891 11.893-11.891 3.181.001 6.167 1.24 8.413 3.488 2.245 2.248 3.481 5.236 3.48 8.414-.003 6.557-5.338 11.892-11.893 11.892-1.99-.001-3.951-.5-5.688-1.448l-6.305 1.654zm6.597-3.807c1.676.995 3.276 1.591 5.392 1.592 5.448 0 9.886-4.434 9.889-9.885.002-5.462-4.415-9.89-9.881-9.892-5.452 0-9.887 4.434-9.889 9.884-.001 2.225.651 3.891 1.746 5.634l-.999 3.648 3.742-.981zm11.387-5.464c-.074-.124-.272-.198-.57-.347-.297-.149-1.758-.868-2.031-.967-.272-.099-.47-.149-.669.149-.198.297-.768.967-.941 1.165-.173.198-.347.223-.644.074-.297-.149-1.255-.462-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.297-.347.446-.521.151-.172.2-.296.3-.495.099-.198.05-.372-.025-.521-.075-.148-.669-1.611-.916-2.206-.242-.579-.487-.501-.669-.51l-.57-.01c-.198 0-.52.074-.792.372s-1.04 1.016-1.04 2.479 1.065 2.876 1.213 3.074c.149.198 2.095 3.2 5.076 4.487.709.306 1.263.489 1.694.626.712.226 1.36.194 1.872.118.571-.085 1.758-.719 2.006-1.413.248-.695.248-1.29.173-1.414z"/></svg>
                        <span>Envoyer preuve via WhatsApp</span>
                      </button>
                      <button
                        onClick={() => {
                          const subject = encodeURIComponent(`Preuve de virement â€” ${currentPlan.name}`);
                          const body = encodeURIComponent(`Bonjour AÃ¯chatou,\n\nVirement effectuÃ© pour : ${currentPlan.name} â€” ${currentPlan.eur} EUR.\n\nCi-joint la preuve de paiement.\n\nCordialement.`);
                          window.location.href = `mailto:moussaousmaneaicha@gmail.com?subject=${subject}&body=${body}`;
                        }}
                      ...
                      >
                        ...
                        <span>Envoyer preuve par Email</span>
                      </button>
                    </div>
                  </div>
                )}
